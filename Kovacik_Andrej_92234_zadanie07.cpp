...
queue<int> reverse(const list<int> & data) noexcept {
    ...
    // prechod kontajnerom pomocou iteratora
    for (i= data.rbegin(); i != data.rend(); ++i) {
        q.push(*i);
    }
...
int sum(istringstream & text) noexcept {
    ...
    return sumSum; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
list<string> translate(const map<string, string> & translator, const list<string> & sentence) noexcept {
    // TODO
    list<string> prelozena {};
    //string slovo ("");
    list<string>::const_iterator i;
    for (i = sentence.begin(); i != sentence.end(); ++i) {
        if (translator.count(*i) == 1) {
            prelozena.push_back(translator.find(*i)->second);
        }
        else {
            prelozena.push_back("?");
        }
    ...
    return prelozena; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
bool areParenthesisPaired(const string & mathExpression) noexcept {
    string expr ("");
    string helperchar (""); // pojebane ccko a zakazane operacie
    ...
    for (size_t i = 0; i < mathExpression.length(); i++) {
        if (mathExpression[i] == '(' || mathExpression[i] == ')' || mathExpression[i] == '{' || mathExpression[i] == '}' || mathExpression[i] == '[' || mathExpression[i] == ']' || mathExpression[i] == '<' || mathExpression[i] == '>') {
            helperchar.assign(1, mathExpression[i]);
            expr.append(helperchar);
        }
    }
...
/*
       1. priklad:
        ...
        3. priklad:
            sentence: { "He", "reads", "a", "book" }, vystup: { "?", "?", "", "kniha"}, poznamka: neprelozil "He", pretoze v slovniku je len "he"
 */
    /*  const map<string, string> slovnik {
              {"read", "citat"},
              {"write", "pisat"},
              {"book", "kniha"},
              {"I", "ja"},
      ...
      list<string> veta1 { "He", "reads", "a", "book" };
      list<string> veta1_prelozea = translate(slovnik, veta1);*/
